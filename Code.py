...
def register_user():
    ...
    if username and email and password and telegram_number:
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute("""
                INSERT INTO users (username, password, email, telegram_number)
                VALUES (?, ?, ?, ?)
...
# Switch to the book purchase screen
...
def switch_to_user_profile(user_email):
    ...
    for purchase in purchases:
        ...
        # Calculate remaining hours until delivery
        try:
...
try:
    original_image = Image.open("/Users/hornnarak/Downloads/BOOKS.jpg")  # Adjust path to your image
...
def login_user(username, email, password):
    if username and email and password:
        ...
        if user:
            ...
            switch_to_purchase_screen(user[2])  
        else:
